...
public class PartyCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, @NotNull String[] args) {
        ...
        switch (args[0]) {
            case "create":
                if (pp.hasParty()) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "이미 가입된 파티가 있습니다.");
                    return false;
                }
                party = new Party(p);
            ...
            case "list":
                if (!pp.hasParty()) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "가입된 파티가 없습니다.");
                    return false;
                }
            ...
            case "remove":
                ...
                party = Party.getParty(pp.getPartyId());
                if (!party.isMaster(p)) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "파티장만 파티를 삭제할 수 있습니다.");
                    return false;
                }
            ...
            case "invite":
                if (args.length < 2) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "초대할 플레이어를 입력해주세요.");
                }
                ...
                party = Party.getParty(pp.getPartyId());
                if (!party.isMaster(p)) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "파티장만 초대할수 있습니다.");
                    return false;
                }

                Player target = Bukkit.getPlayer(args[1]);
                if (target == null) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "초대할 플레이어가 존재하지 않습니다.");
                    return false;
                }

                PartyPlayer pptarget = PartyPlayer.getPartyPlayer(target);
                if (pptarget.hasParty()) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "해당 플레이어는 이미 다른 파티에 속해있습니다.");
                    return false;
                }

                if (inviteMap.containsKey(target.getName())) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "해당 플레이어는 이미 초대를 받았습니다. 잠시후 다시 시도해주세요.");
                    return false;
                }
            ...
            case "kick":
                if (args.length < 2) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "추방할 플레이어를 입력해주세요.");
                }
                ...
                party = Party.getParty(pp.getPartyId());
                if (!party.isMaster(p)) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "파티장만 추방할수 있습니다.");
                    return false;
                }

                target = Bukkit.getPlayer(args[1]);
                if (target == null) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "추방할 플레이어가 존재하지 않습니다.");
                    return false;
                }

                pptarget = PartyPlayer.getPartyPlayer(target);
                if (!pptarget.hasParty() || pptarget.getPartyId() != pp.getPartyId()) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "해당 플레이어는 이 파티에 속해있지 않습니다.");
                    return false;
                }

                if (party.isMaster(target)) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "파티장은 추방할수 없습니다.");
                }
            ...
            case "accept":
                if (!inviteMap.containsKey(p.getName())) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "초대를 받은 기록이 없습니다.");
                    return false;
                }

                if (pp.hasParty()) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "이미 파티에 속해있습니다.");
                    return false;
                }

                party = Party.getParty(inviteMap.get(p.getName()));
                if (party == null) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + "초대를 받은 파티가 존재하지 않습니다. (이미 삭제되었을수 있습니다)");
                    return false;
                }
