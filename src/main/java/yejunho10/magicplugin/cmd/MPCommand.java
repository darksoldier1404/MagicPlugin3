    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, @NotNull String[] args) {
            ...
            try {
                ...
                if (p.hasPermission("myplugin.teleport")) {
                    if (args[1].length() == 0) {
                        p.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "텔레포트 목적지가 설정되지 않았습니다.");
                        return false;
                    } //mp tp
                        ...
                        if (p.isOp()) {
                            if (!args[2].isEmpty()) {
                                ...
                                sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "텔레포트할 플레이어 닉네임을 입력해주세요.");
                                return false;
                            }
                        } else {
                            no_ops(p);
                ...
                for (Player player : Bukkit.getOnlinePlayers()) {
                    if (!player.isOp()) {
                        switch (player.getWorld().getName()) {
                            ...
                            default -> player.sendMessage(ChatColor.YELLOW + "[안내] - " + ChatColor.WHITE + ChatColor.GREEN + player.getName() + ChatColor.LIGHT_PURPLE + "님은 현재, " + ChatColor.GREEN + player.getWorld().getName() + ChatColor.LIGHT_PURPLE + "월드에 있으며, 해당월드는 인식되지 않았으므로 게임모드를 변경하지 못했음을 알려드립니다.");
                        }
                    } else {
                        player.sendMessage(ChatColor.YELLOW + "[안내] - " + ChatColor.WHITE + ChatColor.GREEN + player.getName() + ChatColor.LIGHT_PURPLE + "님은 현재, " + ChatColor.GREEN + player.getWorld().getName() + ChatColor.LIGHT_PURPLE + "월드에 있으며, OP권한이 있어 게임모드가 변경되지 않았음을 알려드립니다.");
                    }
                }
                return true;
            } else {
            ...
            if (sender.isOp()) {
                ...
                if (sender.isOp()) {
                   ...
                   sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "/mp terror <on|off>");
                }
        } //mp terror <on|off>
